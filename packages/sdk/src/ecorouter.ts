...
import type { EcoRouterConfig, EcoRouterExecutionResult, ExecuteRequest } from "./types.js";
import { validateChallenge } from "./x402/validate.js";

type State = { status: "running" } | { status: "complete"; result: EcoRouterExecutionResult } | { status: "failed" | "settlement_unknown" };
...
export class EcoRouter {
  ...
  constructor(private readonly config: EcoRouterConfig) {
  ...
  async execute(request: ExecuteRequest): Promise<EcoRouterExecutionResult> {
    ...
    try {
      ...
      try {
        ...
        const result: EcoRouterExecutionResult = {
          executionId,
          output,
          decision: {
            providerId: winner.providerId,
